...
echo "Running mysqldump within docker container: \n"
docker exec -it ${compose_dirname}-db /bin/sh -c "cd /var/lib/mysql && mysqldump -u ${compose_dirname}_user -p ${compose_dirname}_db > ${backup_filename}"
...
echo "Moving SQL dump file to ${db_backups_dir}:\n"
sudo mv /var/lib/docker/volumes/${compose_dirname}_mysql/_data/${backup_filename} ${db_backups_dir}/
echo "Giving you ($USER) ownership on ${db_backups_dir}/*:\n"
sudo chown -R $USER:$USER ${db_backups_dir}/

echo "Creating simbolic link to latest backup:\n"
