...
if [ ! -d "$gitserverhooksdir" ]; then
    echo "Directory $gitserverhooksdir DOES NOT exists.";
...
sed -i -e "s/PRIV_REG_SERVER/$PRIV_REG_SERVER/g" "$gitserverhooksdir/docker-compose.yml"
...
echo "We will try to docker-compose up -d";
