...
if [ ! -d "$registrydir" ]; then
    echo -e "Directory $registrydir DOES NOT exists.";
    mkdir -p "${registrydir}"
fi

if [ ! -d "${registrydir}/auth" ]; then
    echo -e "Directory $registrydir/auth DOES NOT exists.";
    mkdir -p "${registrydir}/auth"
fi

if [ ! -d "${registrydir}/vhost" ]; then
    echo -e "Directory $registrydir/vhost DOES NOT exists.";
...
echo -e "Enableing larger files for larger images";
...
echo -e "You will now decide what password to use for ${username}";
cd "${registrydir}/auth"
htpasswd -Bc registry.password $username

echo -e "Do not forget to login before pushing with:\n docker login $fqdn\n";
echo -e "IMPORTANT: do NOT put the :5000 at the end of login domain\n";
echo -e "You can now: docker-build-push -t $fqdn/$username/myapp-react:0.0.2";

# we need to destroy previously running nginx-proxy container
# in order to load the vhost for larger files
echo -e "Destroying let's encrypt nginx, in order to load new vhost";
docker rm -f nginx-proxy
docker rm -f nginx-proxy-gen
docker rm -f nginx-proxy-le

echo -e "Reloading nginx-proxy, it should have the vhost data";
cd $HOME/dotfiles/scripts/ubuntu-recreate/nginx-proxy
docker-compose up -d

echo -e "We will try to docker-compose up --force-recreate"
cd "${registrydir}"
